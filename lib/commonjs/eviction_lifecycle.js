...
function useMarkAndExpire(rootKey, isEvicted, {
  ...
  const hasBeenEvictedRef = (0, _react.useRef)(false);
  (0, _react.useEffect)(() => {
    if (!enabled) return;
    if (isEvicted) hasBeenEvictedRef.current = true;else if (hasBeenEvictedRef.current) (0, _auto_restorable.reportRestorationStats)();
  }, [enabled, isEvicted]);
}
