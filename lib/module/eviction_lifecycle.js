...
import { reportRestorationStats } from "./auto_restorable.js";
...
function useMarkAndExpire(rootKey, isEvicted, {
  ...
  useEffect(() => {
    if (!enabled) return;
    if (isEvicted) hasBeenEvictedRef.current = true;else if (hasBeenEvictedRef.current) reportRestorationStats();
  }, [enabled, isEvicted]);
}
