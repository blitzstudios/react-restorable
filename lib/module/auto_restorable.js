...
import { isRestorableEqual } from "./equality.js";
import { UNANCHORED_SCOPE_PREFIX, getDetachReason, getIsRestorationDebugEnabled, getIsRestorationEnabled, getRestorationGeneration, readRestorable, registerScopedStore, registerTestReset, reportRestorationOnce, restorationCounters, useRestorationHiddenNode, useRestorationScope, writeRestorable } from "./restorable_state.js";
 ...
 */
export function isRestorable(value) {
  const seen = new Set();
...
export function reportRestorationStats() {
  ...
  console.log(`[restore-stats] restored=${s.restored} changed=${s.restoredChanged} changedSites=${s.restoredChangedSitesDistinct} missed=${s.missed} manualOk=${s.manualRestored} manualMiss=${s.manualMissed} pruned=${s.pruned} unanchored=${s.refusedUnanchored} contended=${s.refusedContended} evicted=${s.evicted} values=${s.values} live=${s.live}`);
 ...
 */
export function useScopedKey(id, store, enabled, onDetach) {
  ...
  const refusalCountedRef = useRef(null);
  ...
  useEffect(() => {
    ...
    return () => {
      if (contendedKeys.has(key) || getDetachReason(hiddenNode, key, generation) === 'removed') store.delete(key);else onDetach?.(key, generation);
      releaseKey(key);
    };
 ...
 */
export function useRestorationFrame(frameId) {
  'use no memo';
...
export function useAutoState(id, initialValue) {
  'use no memo';

  const frame = useRestorationFrame(id);
  return frame.state(0, useState(frame.initial(0, initialValue)));
}
