import { getIsRestorationDebugEnabled, registerTestReset } from './restorable_state';
...
export type Snapshot = { uri: string; capturedAt: number; sequence: number };
...
let configured: SnapshotCapture | undefined;

/** Set once, at launch. Unset, nothing is photographed and every snapshot option is inert. */
export function configureRestorationSnapshots(capture: SnapshotCapture) {
  configured = capture;
}

export function isSnapshotCaptureConfigured() {
...
export function currentCaptureSequence() {
...
function snapshotKey(rootKey: string, place: string) {
  return `${rootKey}|${place}`;
}

function report(message: string) {
  // eslint-disable-next-line no-console
  if (getIsRestorationDebugEnabled()) console.log(`[restore-snapshot] ${message}`);
}

function dropSnapshot(key: string) {
...
function fileSnapshot(key: string, uri: string) {
  captureSequence += 1;
  // Re-inserted so the map orders by recency, since `set` on an existing key leaves it in place.
  dropSnapshot(key);
  snapshots.set(key, { uri, capturedAt: Date.now(), sequence: captureSequence });
...
export async function captureSnapshot(rootKey: string, place: string, view: unknown) {
...
export function peekSnapshot(rootKey: string, place: string, afterSequence = -1) {
...
export function discardRestorationSnapshots(rootKey: string) {
...
registerTestReset(clearSnapshots);

/** Files a picture as if it had just been taken. */
export function seedSnapshotForTests(rootKey: string, place: string, uri: string) {
  fileSnapshot(snapshotKey(rootKey, place), uri);
}

export function resetSnapshotCaptureForTests() {
