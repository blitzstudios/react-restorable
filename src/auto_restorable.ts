...
const valueStore = new Map<string, RestorableEntry<Map<number, unknown>>>();

registerScopedStore(valueStore);
...
function isPlainContainer(value: object) {
 ...
 */
export function isRestorable(value: unknown) {
...
const contendedKeys = new Set<string>();

/** Per mount, so the counters stay comparable. */
const stats = { restored: 0, restoredChanged: 0, missed: 0, refusedUnanchored: 0, refusedContended: 0, evicted: 0 };

const refusedUnanchoredIds = new Set<string>();
const refusedContendedKeys = new Set<string>();
/** `frameId:slot` → restores that differed from the initial value, which is what a user can actually see. */
const restoredChangedSites = new Map<string, number>();
...
const reportOnce = reportRestorationOnce;
...
export function getRestoredChangedSites(limit = 20) {
...
export function useScopedKey(
  id: string,
  store: Map<string, RestorableEntry<any>>,
  enabled: boolean,
  onDetach?: (key: string, generation: number) => void,
) {
  const scope = useRestorationScope();
  const hiddenNode = useRestorationHiddenNode();
  const isAnchored = enabled && getIsRestorationEnabled() && !scope.startsWith(UNANCHORED_SCOPE_PREFIX);
  const key = `${scope}|${id}`;
  // Refusal is decided during render, so this keeps the count per-mount rather than per-render.
  const refusalCountedRef = useRef<string | null>(null);
  const countRefusalOnce = (outcome: 'unanchored' | 'contended') => {
  ...
  useEffect(() => {
    ...
    return () => {
      if (contendedKeys.has(key) || getDetachReason(hiddenNode, key, generation) === 'removed') store.delete(key);
      else onDetach?.(key, generation);
      releaseKey(key);
    };
  }, [key, isAnchored, store, hiddenNode, onDetach]);

  if (!isAnchored) {
    if (enabled && getIsRestorationEnabled() && countRefusalOnce('unanchored')) {
...
type PartialValue = { __restorablePartial: true; fields: Record<string, unknown> };

function isPartialValue(value: unknown): value is PartialValue {
  return typeof value === 'object' && value !== null && (value as PartialValue).__restorablePartial === true;
}
 ...
 */
function toStorable(value: unknown): { value: unknown } | undefined {
  if (!isPlainRecord(value)) return isRestorable(value) ? { value } : undefined;
  ...
  return kept === 0 ? undefined : { value: { __restorablePartial: true, fields } satisfies PartialValue };
}

function differsFromInitial(stored: unknown, initial: unknown) {
  if (!isPartialValue(stored)) return !isRestorableEqual(stored, initial);
  const initialRecord = (initial ?? {}) as Record<string, unknown>;
  return Object.entries(stored.fields).some(([field, value]) => !isRestorableEqual(value, initialRecord[field]));
}

function resolveInitial(init: unknown) {
...
const INERT_FRAME: RestorationFrameApi = {
  initial: (_slot, init) => init,
  state: (_slot, tuple) => tuple,
};

class RestorationFrame implements RestorationFrameApi {
  key: string | null;

  private readonly frameId: string;
  private restored: Map<number, unknown> | undefined;
  ...
  initial<T>(slot: number, init: T | (() => T)): T | (() => T) {
    ...
    return (isPartialValue(stored) ? { ...(initial as object), ...stored.fields } : stored) as T;
  }
  ...
  attach(hiddenNode: ReturnType<typeof useRestorationHiddenNode>) {
    const key = this.key;
    ...
    return () => {
      if (!contendedKeys.has(key) && getDetachReason(hiddenNode, key, generation) !== 'removed') this.snapshot(key, generation);
      releaseKey(key);
    };
  }

  /** Runs once per eviction or hide, which is the only moment the values are needed. */
  private snapshot(key: string, generation: number) {
    ...
    writeRestorable(valueStore, key, values, generation);
...
function useLiveRestorationFrame(frameId: string): RestorationFrameApi {
  ...
  useEffect(() => frame.attach(hiddenNode), [frame, key, hiddenNode]);
 ...
 */
export function useRestorationFrame(frameId: string): RestorationFrameApi {
  'use no memo';
...
export function useAutoState<T>(id: string, initialValue: T | (() => T)) {
  'use no memo';

  const frame = useRestorationFrame(id);
  return frame.state(0, useState<T>(frame.initial(0, initialValue)));
}
