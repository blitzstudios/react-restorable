...
import { useScopedKey } from '../auto_restorable';
...
const offsetStore = new Map<string, RestorableEntry<{ x: number; y: number }>>();
...
const HELD_STYLE = { transform: [{ translateX: HELD_OFFSET }], pointerEvents: 'none' } as const;
...
function isNativeHandler(handler: unknown) {
  if (typeof handler !== 'function' && typeof handler !== 'object') return false;
  if (handler === null) return false;
  const candidate = handler as { __isNative?: boolean; __workletHash?: number; workletEventHandler?: unknown };
  return Boolean(candidate.__isNative || candidate.__workletHash || candidate.workletEventHandler);
}

function applyOffset(instance: Instance | null, kind: Kind, offset: { x: number; y: number }, horizontal: boolean) {
  if (!instance) return;
  if (kind === 'scrollTo') {
    instance.scrollTo?.({ x: offset.x, y: offset.y, animated: false });
    return;
  }
  instance.scrollToOffset?.({ offset: horizontal ? offset.x : offset.y, animated: false });
}

function readOffset(event: { nativeEvent?: { contentOffset?: { x?: number; y?: number } } } | undefined) {
 ...
 */
function useRowNamespace(
  renderItem: ((info: any) => React.ReactElement | null) | undefined,
  keyExtractor: ((item: any, index: number) => string) | undefined,
  restoreId: string | undefined,
  isActive: boolean,
): ((info: any) => React.ReactElement | null) | undefined {
  return React.useMemo(() => {
    if (!isActive || !renderItem || !keyExtractor || !restoreId) return undefined;

    const NamespacedRow = (info: any) => {
      const rendered = renderItem(info);
      if (rendered === null || rendered === undefined) return rendered;
      return (
        <RestorationNamespace name={`${restoreId}[${keyExtractor(info.item, info.index)}]`}>{rendered}</RestorationNamespace>
      );
    };
...
export function withScrollRestoration<C extends React.ComponentType<any>>(WrappedComponent: C, kind: Kind): C {
  const Restoring = React.forwardRef(function ScrollRestoring({ restoreId, ...props }: any, forwardedRef: any) {
    ...
    const initialContentOffsetRef = useRef<{ x: number; y: number } | undefined>(undefined);
  ...
  const Enhanced = React.forwardRef(function WithScrollRestoration({ __restoreId: restoreId, ...props }: any, forwardedRef: any) {
    if (!restoreId || !getIsRestorationEnabled()) return <WrappedComponent ref={forwardedRef} {...props} />;
    return <Restoring ref={forwardedRef} restoreId={restoreId} {...props} />;
  });
